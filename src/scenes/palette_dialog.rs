...
impl PaletteDialog {
    pub fn new(colors: Vec<Color>, width: usize, height: usize, style: &DialogStyle) -> Box<Self> {
        ...
        let dos = Button::new(button_start_pos + (button_offset_x,0),"DOS",Some(60), &style.button);
        let gb = Button::new(button_start_pos +(button_offset_x*2,0),"GB",Some(60), &style.button);
        let pico = Button::new(button_start_pos+ (0,button_offset_y),"Pico-8",Some(60), &style.button);
        let zx = Button::new(button_start_pos+ (button_offset_x,button_offset_y),"ZX",Some(60), &style.button);
        let def = Button::new(button_start_pos+ (button_offset_x*2,button_offset_y),"Default",Some(60), &style.button);

        let pal_but_y = 70;
        let edit = Button::new((button_start_pos.x, dialog_pos.y+pal_but_y),"Replace",Some(60), &style.button);
        let delete = Button::new((button_start_pos.x + button_offset_x, dialog_pos.y+pal_but_y),"Delete",Some(60), &style.button);
...
impl Scene<SceneResult, SceneName> for PaletteDialog {
    fn render(&self, graphics: &mut Graphics, mouse_xy: Coord) {
        ...
        graphics.draw_text("R", TextPos::px(self.dialog_pos + (60,96)), (WHITE, Normal));
        graphics.draw_text("G", TextPos::px(self.dialog_pos + (60,108)), (WHITE, Normal));
        graphics.draw_text("B", TextPos::px(self.dialog_pos + (60,120)), (WHITE, Normal));

        let mut y = 0;
        let mut x = 0;
        for (i, color) in self.colors.iter().enumerate() {
            graphics.draw_rect(Rect::new_with_size(self.dialog_pos+PAL_POS + (x * PAL_SPACED,y*PAL_SPACED), PAL_TILE_SIZE, PAL_TILE_SIZE), fill(*color));
            if i == self.selected_color {
                graphics.draw_rect(Rect::new_with_size(self.dialog_pos+PAL_POS + (x * PAL_SPACED,y*PAL_SPACED) -(1,1), PAL_TILE_SIZE+2, PAL_TILE_SIZE+2), stroke(WHITE));
            }
            x+=1;
            if x > PAL_PER_ROW {
                x =0 ;
    ...
    fn on_mouse_click(&mut self, xy: Coord, _: &Vec<&VirtualKeyCode>) {
        ...
        if self.add.on_mouse_click(xy) {
            if self.colors.len() < 42 {
                self.colors.push(self.current_color);
            }
        }
        if self.delete.on_mouse_click(xy) {
