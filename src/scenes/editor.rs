...
impl Editor {
    pub fn new(details: EditorDetails,width: usize,height: usize, alert_style: &AlertStyle,button_style: &ButtonStyle) -> Box<Self> {
        let (file_path, image): (Option<String>, IndexedImage) = match details {
            EditorDetails::Open(path) => {
                match fs::read_to_string(&path) {
                    Ok(data) => (Some(path), ron::from_str(&data).expect("Could not decode ici file in ron format")),
                    Err(e) => panic!("opening {path}:  {}",e)
                }
            }
            EditorDetails::New(w, h) => (None, IndexedImage::new_empty(palettes::Palette::default().colors, w, h))
        };
        ...
        let file_name = file_path.as_ref().map(|p| PathBuf::from(p).file_name().unwrap().to_string_lossy().to_string()).unwrap_or(String::from("Untitled"));
        let tile_size = (CANVAS_WIDTH / image.width().max(image.height())).min(16).max(4);
        ...
        InsertShape::insert_above(&mut transparent_placeholder, Rect::new_with_size((0,0),trans_size,trans_size),fill(LIGHT_GRAY));
        InsertShape::insert_above(&mut transparent_placeholder, Rect::new_with_size((trans_size,0),trans_size,trans_size),fill(DARK_GRAY));
        InsertShape::insert_above(&mut transparent_placeholder, Rect::new_with_size((0,trans_size),trans_size,trans_size),fill(DARK_GRAY));
        InsertShape::insert_above(&mut transparent_placeholder, Rect::new_with_size((trans_size,trans_size),trans_size,trans_size),fill(LIGHT_GRAY));
...
impl Scene<SceneResult, SceneName> for Editor {
    fn render(&self, graphics: &mut Graphics, mouse_xy: Coord) {
        ...
        graphics.draw_text(&self.file_name, TextPos::px(FILENAME_POS), (WHITE, Normal, WrappingStrategy::Ellipsis(38)));
        graphics.draw_line((0, FILENAME_POS.y + 10),(WIDTH as isize, FILENAME_POS.y + 10),LIGHT_GRAY);
        ...
        for x in 0..self.image.width() {
            for y in 0..self.image.height() {
                ...
                if color != TRANSPARENT {
                    graphics.draw_rect(Rect::new_with_size(xy, self.tile_size, self.tile_size), fill(color));
                }
            }
        }
    ...
    fn on_key_press(&mut self, key: VirtualKeyCode, held_keys: &Vec<&VirtualKeyCode>) {
        if (held_keys.contains(&&VirtualKeyCode::LControl) || held_keys.contains(&&VirtualKeyCode::LWin)) && key == VirtualKeyCode::Z {
            todo!("undo");
        } else if (held_keys.contains(&&VirtualKeyCode::LControl) || held_keys.contains(&&VirtualKeyCode::LWin)) && key == VirtualKeyCode::Y {
            todo!("redo");
        }
    }
    ...
    fn resuming(&mut self, result: Option<SceneResult>) {
        match result {
            Some(SceneResult::SaveFilePath(path)) => {
                self.file_name = PathBuf::from(&path).file_name().unwrap().to_string_lossy().to_string();
                self.file_path = Some(path);
                self.save_image();
            }
            Some(SceneResult::Palette(mut colors)) => {
                self.image.apply_new_colors(colors);
                self.selected_color_idx = self.selected_color_idx.min(self.image.colors().len() - 1);
            }
            _ => {}
        }
        self.result = Nothing;
